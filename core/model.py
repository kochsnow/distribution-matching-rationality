...
class TargetRNN(tf.keras.Model):
    ...
    def __init__(self, args):
        ...
        self.dis_embedding_layer = Embedding(args.vocab_size,
                                             args.embedding_dim,
                                             args.pretrained_embedding)
#         self.dis_teacher_embedding_layer = Embedding(args.vocab_size,
#                                              args.embedding_dim,
#                                              args.pretrained_embedding)
        ...
        #self.discriminator_teacher = RNN(args.cell_type, args.hidden_dim)
        # discriminator output layer (classification task)
        self.discriminator_fc = tf.keras.layers.Dense(units=args.num_classes)
        #self.discriminator_teacher_fc = tf.keras.layers.Dense(units=args.num_classes)
...
class TeacherEncoder(tf.keras.Model):
    """A RNN-based Target Dependent Rationalization Model."""
    ...
    def call(self, inputs, masks, labels, path):
        ...
        dis_embeddings = masks_ * self.dis_teacher_embedding_layer(inputs)
        discriminator_teacher_outputs = self.discriminator_teacher(dis_embeddings)

        # mask before max pooling
        masked_dis_teacher_outputs = discriminator_teacher_outputs * masks_ + (1. - masks_) * (-1e6)

        # aggregates hidden outputs via max pooling
        discriminator_teacher_output = tf.reduce_max(masked_dis_teacher_outputs, axis=1)
        # task prediction
        # shape -- (batch_size, num_classes)
        discriminator_teacher_logits = self.discriminator_teacher_fc(discriminator_teacher_output)
        #pdb.set_trace()
        return discriminator_teacher_logits, discriminator_teacher_output
    
    
    def discriminator_teacher_trainable_variables(self):
        """
        Return a list of trainable variables of the discriminator.
        """
        variables = self.discriminator_teacher.trainable_variables + self.discriminator_teacher_fc.trainable_variables
        variables += self.dis_teacher_embedding_layer.trainable_variables
        return variables